...
| Variable selection (descent driver) | fixed/static mix of VSIDS / MRV / dom-wdeg | activity (drives most descent picks; tendency-control all-off marginal +21 **[primary ②]**, isolated +81 **[primary ①]** = the real workhorse; marginal/isolated → §1 box) + **bandit-learned mix** (mix_p, directional) |
...
One scope note, stated plainly. That "small pure-pruning" is partly a *consequence of deliberately choosing the weakest learning clause*: a conjunction of decision literals only prunes that one exact combination, so a small pure-pruning term is almost by design. So Section 3 shows "how far tendency alone carries you once logic is deliberately minimized" — **not** that LCG's deductive pruning is generally redundant. On classes where strong learning is decisive — problems where 1-UIP / LCG prune by orders of magnitude — tendency alone won't reach, and this article didn't measure that. Bounding the claim that way is what keeps the contrast in its correct scope.
