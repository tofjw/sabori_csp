...
PresolveResult IntLinNeReifConstraint::presolve(Model& model) {
    bool changed = false;
...
void IntLinNeReifConstraint::save_trail_if_needed(Model& model, int save_point) {
    if (trail_.save_if_needed(save_point, {current_fixed_sum_, min_rem_potential_, max_rem_potential_, unfixed_count_})) {
...
bool IntLinNeReifConstraint::on_set_min(Model& model, int save_point,
    ...
    if (model.is_instantiated(b_id_)) {
        if (model.value(b_id_) == 1 && min_sum == target_ && max_sum == target_) return false;
        if (model.value(b_id_) == 0 && (target_ < min_sum || target_ > max_sum)) return false;
    } else {
        if (target_ < min_sum || target_ > max_sum) {
            model.enqueue_instantiate(b_id_, 1);
        } else if (min_sum == target_ && max_sum == target_) {
...
void IntLinNeReifConstraint::init_activity(const Model& model, double* activity) const {
    int64_t max_abs = 0;
    for (auto c : coeffs_) {
        int64_t a = c < 0 ? -c : c;
