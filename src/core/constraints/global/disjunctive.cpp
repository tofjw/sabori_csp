...
DisjunctiveConstraint::DisjunctiveConstraint(
    ...
    , strict_(strict)
{
    // var_ids_ = [s0, s1, ..., sn-1, d0, d1, ..., dn-1]
    std::vector<VariablePtr> all_vars;
    all_vars.reserve(2 * n_);
    ...
    if (n_ > 0) {
        int64_t min_start = all_vars[0]->min();
        int64_t max_end   = all_vars[0]->max() + all_vars[n_]->max();
        for (size_t i = 1; i < n_; ++i) {
            min_start = std::min(min_start, all_vars[i]->min());
            int64_t end_i = all_vars[i]->max() + all_vars[n_ + i]->max();
...
bool DisjunctiveConstraint::task_fully_assigned(const Model& model, size_t task) const {
    return model.is_instantiated(var_ids_[task]) && model.is_instantiated(var_ids_[n_ + task]);
}

int DisjunctiveConstraint::task_start(const Model& model, size_t task) const {
    return static_cast<int>(model.value(var_ids_[task]));
}

int DisjunctiveConstraint::task_dur(const Model& model, size_t task) const {
    return static_cast<int>(model.value(var_ids_[n_ + task]));
}

int DisjunctiveConstraint::task_dur_min(const Model& model, size_t task) const {
    return static_cast<int>(model.var_min(var_ids_[n_ + task]));
}
...
bool DisjunctiveConstraint::update_compulsory_part_direct(Model& model, size_t task) {
    int dur = task_dur_min(model, task);
    if (dur <= 0) return true;
    if (!strict_ && dur == 0) return true;

    int est = static_cast<int>(model.variable(var_ids_[task])->min()) - offset_;
    int lst = static_cast<int>(model.variable(var_ids_[task])->max()) - offset_;
...
bool DisjunctiveConstraint::edge_finding(Model& model, bool direct) {
    ...
    for (size_t i = 0; i < n_; ++i) {
        ...
        int est = static_cast<int>(model.var_min(var_ids_[i])) - offset_;
        int lst = static_cast<int>(model.var_max(var_ids_[i])) - offset_;
    ...
    for (size_t k = 0; k < m; ++k) {
        ...
        for (size_t j = k + 1; j < m; ++j) {
            ...
            if (model.is_instantiated(j_s_id)) continue;
            ...
            if (effective_L + energy_theta + dur_j + ext_for_j > R) {
                ...
                if (new_est > est_j) {
                    ...
                    if (direct) {
                        if (!model.variable(var_ids_[tasks[j].idx])->remove_below(new_min)) return false;
                    } else {
        ...
        for (size_t j = k + 1; j < m; ++j) {
            ...
            if (L + energy_theta + dur_j + ext_for_j > effective_R) {
                ...
                if (new_lst < tasks[j].lst) {
                    ...
                    if (direct) {
                        if (!model.variable(var_ids_[tasks[j].idx])->remove_above(new_max)) return false;
                    } else {
    ...
    if (!strict_) {
        for (size_t i = 0; i < n_; ++i) {
            auto d_id = var_ids_[n_ + i];
            int dur_min_i = static_cast<int>(model.var_min(d_id));
            if (dur_min_i != 0) continue;
            if (model.is_instantiated(d_id)) continue;
            ...
            if (!can_fit) {
                if (direct) {
                    if (!model.variable(var_ids_[n_ + i])->assign(0)) return false;
                } else {
...
bool DisjunctiveConstraint::presolve(Model& model) {
    ...
    for (size_t i = 0; i < n_; ++i) {
        auto s_var = model.variable(var_ids_[i]);
        auto d_var = model.variable(var_ids_[n_ + i]);
        if (s_var->is_assigned()) continue;

        int dur;
        if (d_var->is_assigned()) {
            dur = task_dur(model, i);
        } else {
            dur = task_dur_min(model, i);
        }

        if (dur == 0 && !strict_) {
            if (!d_var->is_assigned() && d_var->domain().size() > 1) {
...
bool DisjunctiveConstraint::on_final_instantiate(const Model& model) {
    // 全タスクペアの非重複を確認
    for (size_t i = 0; i < n_; ++i) {
        auto si = task_start(model, i);
        auto di = task_dur(model, i);
