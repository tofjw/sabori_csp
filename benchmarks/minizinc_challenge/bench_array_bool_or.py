#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: array_bool_or 制約を含む問題のみ
(MiniZinc Challenge 2022/2023/2024/2025 problems)

array_bool_or は FlatZinc レベルの制約なので、MiniZinc モデル (.mzn) ではなく
...
def has_array_bool_or(prob_dir):
    """sabori_csp 用の FlatZinc を生成して array_bool_or 制約が含まれるか検出する。

    sabori_csp の redefinitions.mzn を使って compile した結果のみを信頼する。
    既存の .fzn ファイルは他ソルバー用の可能性があるため使用しない。
    また "constraint array_bool_or(" で厳密マッチし、部分文字列の誤検出を防ぐ。
    """
    ...
    cmd.extend(["-o", "/tmp/_check_array_bool_or.fzn"])

    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        if r.returncode == 0:
            content = Path("/tmp/_check_array_bool_or.fzn").read_text()
            return "constraint array_bool_or(" in content
    except Exception:
        pass
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_array_bool_or_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: array_bool_or Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: array_bool_or Problems</h1>
    <div class="meta">
        Generated: {now} | Timeout: {TIMEOUT}s | Problems: {len(problems)} (from MiniZinc Challenge 2022/2023/2024/2025)
    </div>
...
def main():
    ...
    print(f"Found {len(problems)} problems with array_bool_or constraint:")
    for prob in problems:
