#!/usr/bin/env python3
"""one-hot チャネル集約 presolve A/B test（集約が発火する問題のみ）。

Same binary, toggled via env SABORI_ONEHOT:
  - onehot_on  : SABORI_ONEHOT=1  -> 同一変数の int_eq_reif 群を IntOneHotChannel に融合（既定動作）
  - onehot_off : SABORI_ONEHOT=0  -> 集約せず（個別 reif のまま）

測りたい問い: one-hot 集約（記事8章）は探索性能に寄与しているか。
集約が発火しない問題では on/off が同一になるため、-v プローブで発火を検出した問題だけ計測する。
...
PAIRS = [
    ("onehot_on", "onehot_off"),  # one-hot 集約の寄与
]
...
def main():
    ...
    # 集約が発火する問題のみ残す（-v プローブ）
    def aggregates(fzn):
        try:
            r = subprocess.run([BINARY, "-v", "-t", "2", fzn], capture_output=True, text=True, timeout=20)
            return "OneHotChannelAggregator:" in (r.stderr or "")
        except Exception:
            return False
    agg_problems = [p for p in problems if aggregates(fzn_map[p])]
    dropped = [p for p in problems if p not in agg_problems]
    if dropped:
        print(f"Excluding {len(dropped)} problems with no aggregation: {dropped}")
    problems = agg_problems
    print(f"Aggregating problems: {len(problems)}")
