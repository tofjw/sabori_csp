#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: bool_clause 制約を含む問題のみ
(MiniZinc Challenge 2022/2023/2024/2025 problems)

bool_clause は FlatZinc レベルの制約なので、MiniZinc モデル (.mzn) ではなく
...
def has_bool_clause(prob_dir):
    """FlatZinc を生成して bool_clause 制約が含まれるか検出する。
    ...
    cmd.extend(["-o", "/tmp/_check_bool_clause.fzn"])

    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        if r.returncode == 0:
            content = Path("/tmp/_check_bool_clause.fzn").read_text()
            return "bool_clause" in content
    except Exception:
        pass
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_bool_clause_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: bool_clause Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: bool_clause Problems</h1>
    <div class="meta">
...
def main():
    ...
    print(f"Found {len(problems)} problems with bool_clause constraint:")
    for prob in problems:
