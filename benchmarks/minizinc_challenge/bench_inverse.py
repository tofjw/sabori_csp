#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: inverse 制約を含む問題のみ
...
def has_inverse(prob_dir):
    """問題ディレクトリが inverse を使用しているか"""
    prob_dir = Path(prob_dir)
    for f in prob_dir.glob("*.mzn"):
        try:
            content = f.read_text()
            if re.search(r'\binverse\b', content):
                return True
        except Exception:
            pass
    return False
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_inverse_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: inverse Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: inverse Problems</h1>
    <div class="meta">
...
def main():
    ...
    print(f"Found {len(problems)} problems with inverse constraint:")
    for prob in problems:
