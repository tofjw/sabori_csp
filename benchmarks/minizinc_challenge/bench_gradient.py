#!/usr/bin/env python3
"""擬似勾配ヒント A/B test（最適化問題のみ）。

Same binary, toggled via env SABORI_GRADIENT:
  - grad_on  : SABORI_GRADIENT=1  -> 改善解の差分から推定した勾配方向に値順序を寄せる（既定動作）
  - grad_off : SABORI_GRADIENT=0  -> 勾配ヒント無効（solution-guided 値順序にフォールバック）

測りたい問い: 擬似勾配ヒント（記事7章）は最適化性能に寄与しているか。
勾配は目的関数がある最適化問題でのみ発火するため、SAT 問題は除外する。

各 .mzn を minizinc で .fzn にコンパイルし、fzn_sabori を直接実行して比較する。
"""
...
# pairwise comparisons to tally: (cfgA, cfgB) -> reports [A_win, B_win, tie]
PAIRS = [
    ("grad_on", "grad_off"),  # 擬似勾配ヒントの寄与
]
...
def run_fzn_solver(prob, cfg_name, bump_val, seed, fzn_path):
...
def compute_tightness(fzn_path):
    """fzn から構造的タイトネスの代理指標を計算: 制約宣言行数 / 変数宣言行数。
    高いほど tight（変数あたりの制約が多い）。array 宣言は1行でカウントするため近似。"""
    try:
        with open(fzn_path) as f:
            txt = f.read()
    except OSError:
        return (0, 0, 0.0)
    n_c = len(re.findall(r'(?m)^\s*constraint\b', txt))
    n_v = len(re.findall(r'(?m)^\s*(?:array\b[^;]*of\s+)?var\b', txt))
    ratio = n_c / max(n_v, 1)
    return (n_c, n_v, ratio)

def tally_subset(results, prob_types, problems_subset, ca, cb):
    """problems_subset 上で (ca vs cb) を (問題xシード) 集計。returns ([w,l,t], per_seed)。"""
    total = [0, 0, 0]
    per_seed = []
    for seed in SEEDS:
        s = [0, 0, 0]
        for prob in problems_subset:
...
def main():
    ...
    tightness = {p: compute_tightness(fzn_map[p]) for p in problems if p in fzn_map}
    # 勾配は最適化問題でのみ発火 → SAT を除外
    opt_problems = [p for p in problems if prob_types.get(p) in ("MIN", "MAX")]
    dropped = [p for p in problems if p not in opt_problems]
    if dropped:
        print(f"Excluding {len(dropped)} non-optimization problems: {dropped}")
    problems = opt_problems
    print(f"Compiled {len(problems)} optimization problems (SAT excluded).")

    tasks = []
    for prob in problems:
        for cfg_name, bump_val in CONFIGS:
            for seed in SEEDS:
                tasks.append((prob, cfg_name, bump_val, seed, fzn_map[prob]))
    ...
    # ---- 全体 pairwise tally ----
    print("\n" + "=" * 90)
    total, per_seed = tally_subset(results, prob_types, problems, ca, cb)
    w, l, t = total
    print("PAIRWISE (全最適化問題, 問題 x シード):")
    print(f"  {ca} vs {cb}:  {ca}_win={w}  {cb}_win={l}  tie={t}   net={w-l:+d}")
    for seed, s in zip(SEEDS, per_seed):
        print(f"      seed={seed}: {ca}={s[0]} {cb}={s[1]} tie={s[2]}")

    # ---- 問題別: tightness と勾配の勝敗（5シード集計） ----
    print("\n" + "=" * 90)
    print("PER-PROBLEM (tightness=制約行/変数行, 勾配の5シード勝敗):")
    print(f"{'problem':28s} {'type':4s} {'n_c':>6s} {'n_v':>6s} {'ratio':>6s}   {ca}/{cb}/tie")
    prob_verdict = {}
    for prob in sorted(problems, key=lambda p: tightness.get(p, (0,0,0))[2]):
        nc, nv, ratio = tightness.get(prob, (0, 0, 0.0))
        sub, _ = tally_subset(results, prob_types, [prob], ca, cb)
        prob_verdict[prob] = sub
        print(f"{prob:28s} {prob_types.get(prob,'?'):4s} {nc:>6d} {nv:>6d} {ratio:>6.2f}   {sub[0]}/{sub[1]}/{sub[2]}")

    # ---- loose / tight 二分（ratio 中央値で分割）して群別集計 ----
    ranked = sorted(problems, key=lambda p: tightness.get(p, (0,0,0))[2])
    half = len(ranked) // 2
    loose = ranked[:half]   # ratio 低 = loose
    tight = ranked[half:]   # ratio 高 = tight
    print("\n" + "=" * 90)
    print("LOOSE vs TIGHT (制約/変数比の中央値で二分):")
    for label, subset in [("LOOSE (ratio低)", loose), ("TIGHT (ratio高)", tight)]:
        tot, ps = tally_subset(results, prob_types, subset, ca, cb)
        w, l, t = tot
        nets = [s[0]-s[1] for s in ps]
        print(f"  {label:16s} ({len(subset)}問): {ca}_win={w}  {cb}_win={l}  tie={t}   net={w-l:+d}   seed別net={nets}")
