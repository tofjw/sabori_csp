#!/usr/bin/env python3
"""temporal_activity（Last Conflict 系・変数選択の第1基準）の ablation。

Same binary, toggled via env SABORI_TEMPORAL:
  - temporal_on  : SABORI_TEMPORAL=1  -> temporal_activity を第1基準に使う（既定動作）
  - temporal_off : SABORI_TEMPORAL=0  -> temporal_activity を無効化（全変数 0 のまま、mix_p/MRV へフォールスルー）

測りたい問い: conflict-directed な第1基準（Last Conflict 一般化, Lecoutre et al. 2009 系）は効いているか。
"""
...
PAIRS = [
    ("temporal_on", "temporal_off"),  # 第1基準 temporal_activity の寄与
]
