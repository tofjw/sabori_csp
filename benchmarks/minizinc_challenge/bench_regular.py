#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: regular 制約を含む問題のみ
...
def has_regular(prob_dir):
    """問題ディレクトリが regular 制約を使用しているか

    include "regular.mzn" だけで実際に呼び出していない問題は除外する。
    """
    prob_dir = Path(prob_dir)
    for f in prob_dir.glob("*.mzn"):
        try:
            content = f.read_text()
            if re.search(r'\bregular\s*\(', content):
                return True
        except Exception:
            pass
    return False
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_regular_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: regular Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: regular Problems</h1>
    <div class="meta">
...
def main():
    ...
    print(f"Found {len(problems)} problems with regular constraint:")
    for prob in problems:
