#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: int_lin_eq_reif 制約を含む問題のみ
(MiniZinc Challenge 2022/2023/2024/2025 problems)

int_lin_eq_reif は FlatZinc レベルの制約なので、MiniZinc モデル (.mzn) ではなく
...
def has_int_lin_eq_reif(prob_dir):
    """FlatZinc を生成して int_lin_eq_reif 制約が含まれるか検出する。
    ...
    if fzn_files:
        try:
            ...
            return "int_lin_eq_reif" in content
        except Exception:
            pass
    ...
    cmd.extend(["-o", "/tmp/_check_int_lin_eq_reif.fzn"])

    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        if r.returncode == 0:
            content = Path("/tmp/_check_int_lin_eq_reif.fzn").read_text()
            return "int_lin_eq_reif" in content
    except Exception:
        pass
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_int_lin_eq_reif_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: int_lin_eq_reif Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: int_lin_eq_reif Problems</h1>
    <div class="meta">
...
def main():
    ...
    print(f"Found {len(problems)} problems with int_lin_eq_reif constraint:")
    for prob in problems:
