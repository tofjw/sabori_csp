#!/usr/bin/env python3
"""NoGood の +効果を「枝刈り」と「activity 寄与」に分離する 3-way ablation。

Same binary, 3 configs (env で切替):
  - ng_full  : 既定            -> NoGood 学習＋伝播（枝刈り）＋ activity bump 全部あり
  - ng_prune : SABORI_NG_NOBUMP=1 -> 学習・枝刈りは維持、NoGood 由来の activity bump だけ止める
  - ng_off   : SABORI_NOGOOD=0  -> NoGood 機構まるごと無効

測りたい問い:
  - ng_full  vs ng_prune : activity 寄与の効果（差があれば bump が効いている）
  - ng_prune vs ng_off   : 純粋な枝刈りの効果
  - ng_full  vs ng_off   : 全体（先の +18 と整合するはず）
"""
...
CONFIGS = [
    ("ng_full", {}),
    ("ng_prune", {"SABORI_NG_NOBUMP": "1"}),
    ("ng_off", {"SABORI_NOGOOD": "0"}),
]
# pairwise comparisons to tally: (cfgA, cfgB) -> reports [A_win, B_win, tie]
PAIRS = [
    ("ng_full", "ng_prune"),   # activity 寄与の効果
    ("ng_prune", "ng_off"),    # 純粋な枝刈りの効果
    ("ng_full", "ng_off"),     # 全体
]
