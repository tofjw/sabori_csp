...
BINARY_NO_EMA = str(BASE_DIR / "../../.claude/worktrees/agent-a4ba93d9/build/src/fzn/fzn_sabori")
...
def judge_winner(a_status, a_time, a_obj, b_status, b_time, b_obj, prob_type="SAT"):
    ...
    if a_ok and b_ok:
        if a_status == "OPTIMAL" and b_status != "OPTIMAL": return "Current", "a-win"
        if b_status == "OPTIMAL" and a_status != "OPTIMAL": return "WithCumul", "b-win"
        if a_obj is not None and b_obj is not None and a_obj != b_obj:
            return ("Current", "a-win") if obj_better(a_obj, b_obj) else ("WithCumul", "b-win")
        if a_time < b_time * 0.8: return "Current", "a-win"
        if b_time < a_time * 0.8: return "WithCumul", "b-win"
        return "Tie", ""
    if a_ok and not b_ok: return "Current", "a-win"
    if b_ok and not a_ok: return "WithCumul", "b-win"
    ...
    if a_has and b_has:
        if a_obj != b_obj:
            return ("Current", "a-win") if obj_better(a_obj, b_obj) else ("WithCumul", "b-win")
        return "Tie", ""
    if a_has and not b_has: return "Current", "a-win"
    if b_has and not a_has: return "WithCumul", "b-win"
    return "", ""
...
def generate_html(results, problems, output_path, prob_types, data_labels, prob_years):
    ...
    for prob in problems:
        a_res = results.get((prob, "Current"), ("?", 0, None))
        b_res = results.get((prob, "WithCumul"), ("?", 0, None))
        ...
        if winner == "Current": a_wins += 1
        elif winner == "WithCumul": b_wins += 1
...
<head>
    <meta charset="UTF-8">
    <title>EMA +cumulative check A/B Test</title>
    <style>
...
<body>
    <h1>EMA +cumulative check A/B Test (cumulative problems)</h1>
    <div class="meta">
        Generated: {now} | Timeout: {TIMEOUT}s | Problems: {len(problems)}<br>
        <b>EMA</b>: int_lin + bool_lin only |
        <b>NoEMA</b>: int_lin + bool_lin + cumulative
    </div>
    <div class="summary">
        <div class="stat-card">
        ...
        <div class="stat-card">
            ...
            <div class="stat-label">NoCurrent Wins</div>
        </div>
        <div class="stat-card">
...
def main():
    for label, path in [("Current", BINARY_EMA), ("WithCumul", BINARY_NO_EMA)]:
    ...
    for prob in problems:
        fzn = fzn_map[prob]
        tasks.append((prob, "Current", BINARY_EMA, fzn))
        tasks.append((prob, "WithCumul", BINARY_NO_EMA, fzn))
    ...
    output_path = str(BASE_DIR / f"sabori_benchmark_cumulative_ema_cumul_{ts}.html")
