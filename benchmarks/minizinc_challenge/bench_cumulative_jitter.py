...
BINARY_JITTER = str(BASE_DIR / "../../build/src/fzn/fzn_sabori")
BINARY_NO_JITTER = str(BASE_DIR / "../../.claude/worktrees/agent-a3799757/build/src/fzn/fzn_sabori")
...
def judge_winner(a_status, a_time, a_obj, b_status, b_time, b_obj, prob_type="SAT"):
    ...
    if a_ok and b_ok:
        if a_status == "OPTIMAL" and b_status != "OPTIMAL": return "Jitter", "a-win"
        if b_status == "OPTIMAL" and a_status != "OPTIMAL": return "NoJitter", "b-win"
        if a_obj is not None and b_obj is not None and a_obj != b_obj:
            return ("Jitter", "a-win") if obj_better(a_obj, b_obj) else ("NoJitter", "b-win")
        if a_time < b_time * 0.8: return "Jitter", "a-win"
        if b_time < a_time * 0.8: return "NoJitter", "b-win"
        return "Tie", ""
    if a_ok and not b_ok: return "Jitter", "a-win"
    if b_ok and not a_ok: return "NoJitter", "b-win"
    ...
    if a_has and b_has:
        if a_obj != b_obj:
            return ("Jitter", "a-win") if obj_better(a_obj, b_obj) else ("NoJitter", "b-win")
        return "Tie", ""
    if a_has and not b_has: return "Jitter", "a-win"
    if b_has and not a_has: return "NoJitter", "b-win"
    return "", ""
...
def generate_html(results, problems, output_path, prob_types, data_labels, prob_years):
    ...
    for prob in problems:
        a_res = results.get((prob, "Jitter"), ("?", 0, None))
        b_res = results.get((prob, "NoJitter"), ("?", 0, None))
        ...
        if winner == "Jitter": a_wins += 1
        elif winner == "NoJitter": b_wins += 1
...
<head>
    <meta charset="UTF-8">
    <title>Activity Jitter A/B Test</title>
    <style>
...
<body>
    <h1>Activity Jitter A/B Test (cumulative problems)</h1>
    <div class="meta">
        Generated: {now} | Timeout: {TIMEOUT}s | Problems: {len(problems)}<br>
        <b>Jitter</b>: activity_inc * uniform(0.9, 1.0) |
        <b>NoJitter</b>: activity_inc (no randomness)
    </div>
    <div class="summary">
        <div class="stat-card">
            <div class="stat-value" style="color: #28a745;">{a_wins}</div>
            <div class="stat-label">Jitter Wins</div>
        </div>
        <div class="stat-card">
            <div class="stat-value" style="color: #dc3545;">{b_wins}</div>
            <div class="stat-label">NoJitter Wins</div>
        </div>
        <div class="stat-card">
...
def main():
    for label, path in [("Jitter", BINARY_JITTER), ("NoJitter", BINARY_NO_JITTER)]:
        if not os.path.isfile(path):
            print(f"ERROR: {label} binary not found: {path}")
            sys.exit(1)
    print(f"Jitter:   {BINARY_JITTER}")
    print(f"NoJitter: {BINARY_NO_JITTER}")
    ...
    for prob in problems:
        fzn = fzn_map[prob]
        tasks.append((prob, "Jitter", BINARY_JITTER, fzn))
        tasks.append((prob, "NoJitter", BINARY_NO_JITTER, fzn))
    ...
    print("-" * 100)
    print(f"Wins: Jitter {a_wins}  /  NoJitter {b_wins}  /  Tie {ties}")

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = str(BASE_DIR / f"sabori_benchmark_cumulative_jitter_{ts}.html")
