#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: array_var_int_element 制約を含む問題のみ
(MiniZinc Challenge 2012-2025 problems)

array_var_int_element は FlatZinc レベルの制約なので、MiniZinc モデル (.mzn) ではなく
FlatZinc (.fzn) を生成して検出する。
"""
import subprocess
import sys
import os
...
def has_array_var_int_element(prob_dir):
    """FlatZinc を生成して array_var_int_element 制約が含まれるか検出する。

    既存の .fzn ファイルがあればそれを使い、なければ minizinc --compile で生成。
    """
    prob_dir = Path(prob_dir)

    # 既存 .fzn を先に確認
    fzn_files = list(prob_dir.glob("*.fzn"))
    if fzn_files:
        try:
            content = fzn_files[0].read_text()
            return "array_var_int_element" in content
        except Exception:
            pass

    # .fzn がなければ compile して確認
    ...
    cmd = [MINIZINC, "--solver", SABORI_MSC, "--compile", str(mzn)]
    if data_files:
        cmd.append(str(data_files[0]))
    cmd.extend(["-o", "/tmp/_check_array_var_int_element.fzn"])

    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        if r.returncode == 0:
            content = Path("/tmp/_check_array_var_int_element.fzn").read_text()
            return "array_var_int_element" in content
    except Exception:
        pass
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if output_path is None:
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = f"sabori_benchmark_array_var_int_element_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: array_var_int_element Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: array_var_int_element Problems</h1>
    <div class="meta">
        Generated: {now} | Timeout: {TIMEOUT}s | Problems: {len(problems)} (from MiniZinc Challenge 2012-2025)
    </div>
...
def main():
    ...
    # array_var_int_element を含む問題を全年度から収集
    problems = []
    prob_years = {}
    ...
    for year, probs_dir in sorted(PROB_DIRS.items()):
        if not probs_dir.exists():
            ...
            continue
        prob_list = sorted(probs_dir.iterdir(), key=lambda x: natural_sort_key(x.name))
        for idx, p in enumerate(prob_list):
            if not p.is_dir():
                continue
            print(f"\r  Checking [{year}] {p.name} ({idx+1}/{len(prob_list)})...", end="", flush=True)
            if has_array_var_int_element(p):
                prob_name = p.name
    ...
    print(f"Found {len(problems)} problems with array_var_int_element constraint:")
    for prob in problems:
    ...
    generate_html(results, problems=problems, prob_types=prob_types,
