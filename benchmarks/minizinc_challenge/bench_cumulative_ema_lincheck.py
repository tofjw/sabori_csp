#!/usr/bin/env python3
"""Gradient EMA A/B test: with vs without EMA-based variable selection.
Uses cumulative problems. Runs fzn_sabori directly."""
...
BINARY_NO_EMA = str(BASE_DIR / "../../.claude/worktrees/agent-ac95f34e/build/src/fzn/fzn_sabori")
...
def judge_winner(a_status, a_time, a_obj, b_status, b_time, b_obj, prob_type="SAT"):
    ...
    if a_ok and b_ok:
        if a_status == "OPTIMAL" and b_status != "OPTIMAL": return "WithLinCheck", "a-win"
        if b_status == "OPTIMAL" and a_status != "OPTIMAL": return "NoLinCheck", "b-win"
        if a_obj is not None and b_obj is not None and a_obj != b_obj:
            return ("WithLinCheck", "a-win") if obj_better(a_obj, b_obj) else ("NoLinCheck", "b-win")
        if a_time < b_time * 0.8: return "WithLinCheck", "a-win"
        if b_time < a_time * 0.8: return "NoLinCheck", "b-win"
        return "Tie", ""
    if a_ok and not b_ok: return "WithLinCheck", "a-win"
    if b_ok and not a_ok: return "NoLinCheck", "b-win"
    ...
    if a_has and b_has:
        if a_obj != b_obj:
            return ("WithLinCheck", "a-win") if obj_better(a_obj, b_obj) else ("NoLinCheck", "b-win")
        return "Tie", ""
    if a_has and not b_has: return "WithLinCheck", "a-win"
    if b_has and not a_has: return "NoLinCheck", "b-win"
    return "", ""
...
def generate_html(results, problems, output_path, prob_types, data_labels, prob_years):
    ...
    for prob in problems:
        a_res = results.get((prob, "WithLinCheck"), ("?", 0, None))
        b_res = results.get((prob, "NoLinCheck"), ("?", 0, None))
        ...
        if winner == "WithLinCheck": a_wins += 1
        elif winner == "NoLinCheck": b_wins += 1
...
<head>
    <meta charset="UTF-8">
    <title>EMA Lin-Check A/B Test</title>
    <style>
...
<body>
    <h1>EMA Lin-Check A/B Test (cumulative problems)</h1>
    <div class="meta">
        Generated: {now} | Timeout: {TIMEOUT}s | Problems: {len(problems)}<br>
        <b>EMA</b>: EMA with int_lin/bool_lin check |
        <b>NoEMA</b>: EMA without constraint check
    </div>
    <div class="summary">
        <div class="stat-card">
            <div class="stat-value" style="color: #28a745;">{a_wins}</div>
            <div class="stat-label">WithLinCheck Wins</div>
        </div>
        <div class="stat-card">
            <div class="stat-value" style="color: #dc3545;">{b_wins}</div>
            <div class="stat-label">NoWithLinCheck Wins</div>
        </div>
        <div class="stat-card">
    ...
    <table>
        <thead><tr><th>Problem</th><th>Year</th><th>EMA</th><th>NoEMA</th><th>Winner</th></tr></thead>
...
def main():
    for label, path in [("WithLinCheck", BINARY_EMA), ("NoLinCheck", BINARY_NO_EMA)]:
    ...
    print(f"\nFound {len(problems)} cumulative problems")
    ...
    for prob in problems:
        fzn = fzn_map[prob]
        tasks.append((prob, "WithLinCheck", BINARY_EMA, fzn))
        tasks.append((prob, "NoLinCheck", BINARY_NO_EMA, fzn))
    ...
    # Console summary
    print("\n" + "=" * 90)
    print(f"{'Problem':<35s} {'Year':>4s} {'EMA':>22s} {'NoEMA':>22s} {'Winner':>10s}")
    print("-" * 100)
    ...
    output_path = str(BASE_DIR / f"sabori_benchmark_cumulative_ema_lincheck_{ts}.html")
