#!/usr/bin/env python3
"""temporal によるマスク仮説の検証: activity bump の効果は temporal ON/OFF で変わるか。

仮説（ユーザー）: §3/§4 の activity bump 計測は全て temporal ON で、temporal が大半の
選択を flag で決めるため activity の効きが過小評価された。temporal を外せば activity 本来の
効果が見えるはず。

同一 run で4構成:
  - full           : 既定（temporal on, activity bump 全 on）
  - full_actoff    : temporal on  + 全 activity bump off
  - t_off          : temporal off + activity bump 全 on
  - t_off_actoff   : temporal off + 全 activity bump off

比較:
  - full vs full_actoff   : temporal ON での activity 効果（既知 +21 相当の再測）
  - t_off vs t_off_actoff : temporal OFF での activity 効果
  後者 >> 前者なら「temporal にマスクされていた」= activity bump は本来もっと効く。
"""
...
CONFIGS = [
    ("full", {}),
    ("full_actoff", {"SABORI_BUMP_MODE": "0", "SABORI_NG_LEARN_BUMP": "0", "SABORI_NG_PROP_BUMP": "0", "SABORI_DECVAR_BUMP": "0"}),
    ("t_off", {"SABORI_TEMPORAL": "0"}),
    ("t_off_actoff", {"SABORI_TEMPORAL": "0", "SABORI_BUMP_MODE": "0", "SABORI_NG_LEARN_BUMP": "0", "SABORI_NG_PROP_BUMP": "0", "SABORI_DECVAR_BUMP": "0"}),
]
# pairwise: full vs each（単体3つ + 全部同時 + 制約bumpも含め全停止）
# 相互補償なら単体≈0・all3_off だけ大。all3_off も小さければ制約bumpが補償 → everything_off で確認。
PAIRS = [
    ("full", "full_actoff"),       # temporal ON での activity 効果
    ("t_off", "t_off_actoff"),     # temporal OFF での activity 効果
]
