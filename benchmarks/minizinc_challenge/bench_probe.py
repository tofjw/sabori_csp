#!/usr/bin/env python3
"""improvement probe（最適化の軽量サブ探索）の ablation。

Same binary, toggled via env SABORI_PROBE:
  - probe_on  : SABORI_PROBE=1  -> best 目的値から ~5% 改善を仮目標に軽量プローブ（既定動作）
  - probe_off : SABORI_PROBE=0  -> プローブを実行しない

測りたい問い: §7 で勾配ヒントは効かなかったが、別機構の improvement probe は効くか。
probe は最適化でのみ発火するので SAT は除外。
"""
...
PAIRS = [
    ("probe_on", "probe_off"),  # improvement probe の寄与
]
...
def main():
    ...
    # probe は最適化でのみ発火 → SAT を除外
    opt = [p for p in problems if prob_types.get(p) in ("MIN", "MAX")]
    dropped = [p for p in problems if p not in opt]
    if dropped:
