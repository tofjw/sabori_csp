#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: cumulative 制約を含む問題のみ
...
def has_cumulative(prob_dir):
    """問題ディレクトリが cumulative を使用しているか"""
    prob_dir = Path(prob_dir)
    for f in prob_dir.glob("*.mzn"):
        try:
            content = f.read_text()
            # コメント行を除外して cumulative を探す
...
def _run_checker(cmd, timeout):
    ...
    Returns: (stdout, returncode) or (None, None) on timeout/error.
    """
...
def verify_solution(mzn, data, solution, timeout=10):
    """saboriの解をGecodeで検証する（fallback: Chuffed）。
...
def run_solver(problem, solver_name, solver_id, mzn, data, prob_type="SAT"):
    ...
    #     全問題で cwd を一定（BASE_DIR）にすれば結果が決定的になる。
...
def judge_winner(s_status, s_time, s_obj, c_status, c_time, c_obj, prob_type="SAT", s_check=None):
    ...
    # 既知の原因: MiniZinc 2.9.5 の -O1 最適化でエイリアス置換時に制約を消失
    # させるバグがあり、FZN レベルでは全制約を満たす解が MZN レベルでは不正
    # になるケースがある（例: mznc2021_probs/connect）。
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_cumulative_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: cumulative Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: cumulative Problems</h1>
    <div class="meta">
...
def main():
    ...
    print(f"Found {len(problems)} problems with cumulative constraint:")
    for prob in problems:
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            print(f"{prob:30s} {solver:8s} -> {status:8s} {elapsed:6.2f}s{obj_str}{err_str}{check_str}")

    # コンソール出力
    print("\n" + "=" * 80)
    print("Summary:")
    print("=" * 80)
    print(f"{'Problem':<35s} {'Year':>4s} {'Sabori':>20s} {'CP-SAT':>20s} {'Check':>8s} {'Winner':>10s}")
    print("-" * 105)
    ...
    for prob in problems:
        ...
        data_str = f" [{dlabel}]" if dlabel else ""
        print(f"{prob_label:<35s} {year:>4s}{data_str:>12s} {s_str:>20s} {c_str:>20s} {check_str:>8s} {winner:>10s}")
