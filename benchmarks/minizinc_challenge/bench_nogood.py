#!/usr/bin/env python3
"""decision-trail NoGood 学習 A/B test。

Same binary, toggled via env SABORI_NOGOOD:
  - ng_on  : SABORI_NOGOOD=1  -> decision-trail NoGood の学習＋伝播あり（既定動作）
  - ng_off : SABORI_NOGOOD=0  -> NoGood 機構まるごと無効（学習も伝播もしない）

測りたい問い: LCG より弱い decision-trail NoGood（記事3章）は、そもそも探索に効いているか。
（Bloom タイブレークが no-op だったのは「activity で決着しタイブレークまで来ない」説の検証も兼ねる。
 NG が効くなら、弱い学習でも枝刈り/activity 寄与で価値を出している、ということ。）
...
PAIRS = [
    ("ng_on", "ng_off"),  # decision-trail NoGood 機構の寄与
]
