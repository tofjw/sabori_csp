#!/usr/bin/env python3
"""bump_activity A/B/C test: 制約側 activity 責任配分の3段 ablation.

Same binary, toggled via env SABORI_BUMP_MODE:
  - structural : SABORI_BUMP_MODE=2  -> 制約ごとの構造特化オーバーライド（既定動作）
  - base       : SABORI_BUMP_MODE=1  -> 基底クラスの poor man's explanation（動いた変数・小ドメイン重み）
  - none       : SABORI_BUMP_MODE=0  -> 制約からの加点なし（activity は decision-trail 学習由来のみ）

測りたい問い:
  - 1->2 (none vs base)       : そもそも制約側の責任配分に意味があるか
  - 2->3 (base vs structural) : 構造特化が generic な poor man's explanation を上回るか（記事4章の核）
...
PAIRS = [
    ("structural", "base"),  # 2->3: 構造特化 vs 基底 poor man's
    ("base", "none"),        # 1->2: 基底 vs 加点なし
    ("structural", "none"),  # 1->3: 全責任配分機構の価値
]
