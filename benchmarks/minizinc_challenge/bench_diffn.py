#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: diffn 制約を含む問題のみ
...
def has_diffn(prob_dir):
    """問題ディレクトリが diffn を使用しているか"""
        ...
        try:
            ...
            for line in content.splitlines():
                ...
                if stripped.startswith('%'):
                    continue
                if re.search(r'\bdiffn(?:_strict)?\s*\(', stripped):
                    return True
                if re.search(r'include\s+"diffn(?:_strict)?\.mzn"', stripped):
                    return True
        except Exception:
            pass
    return False
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_diffn_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: diffn Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: diffn Problems</h1>
    <div class="meta">
...
def main():
    ...
    print(f"Found {len(problems)} problems with diffn constraint:")
    for prob in problems:
