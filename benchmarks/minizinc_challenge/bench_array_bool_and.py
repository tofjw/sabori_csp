#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: array_bool_and 制約を含む問題のみ
(MiniZinc Challenge 2022/2023/2024/2025 problems)

array_bool_and は FlatZinc レベルの制約なので、MiniZinc モデル (.mzn) ではなく
...
def has_array_bool_and(prob_dir):
    """FlatZinc を生成して array_bool_and 制約が含まれるか検出する。
    ...
    if fzn_files:
        try:
            ...
            return "array_bool_and" in content
        except Exception:
            pass
    ...
    cmd.extend(["-o", "/tmp/_check_array_bool_and.fzn"])

    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        if r.returncode == 0:
            content = Path("/tmp/_check_array_bool_and.fzn").read_text()
            return "array_bool_and" in content
    except Exception:
        pass
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_array_bool_and_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: array_bool_and Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: array_bool_and Problems</h1>
    <div class="meta">
...
def main():
    ...
    print(f"Found {len(problems)} problems with array_bool_and constraint:")
    for prob in problems:
