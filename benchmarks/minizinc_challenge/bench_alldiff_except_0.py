#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: alldifferent_except_0 制約を含む問題のみ
...
# 問題ディレクトリ（年ごと）
PROB_DIRS = {str(y): BASE_DIR / f"mznc{y}_probs" for y in range(2012, 2026)}
...
def has_alldifferent_except_0(prob_dir):
    """問題ディレクトリが alldifferent_except_0 / all_different_except_0 を制約として使用しているか

    include だけで実際に呼び出していない問題は除外する。
    """
    ...
    for f in prob_dir.glob("*.mzn"):
        try:
            content = f.read_text()
            if re.search(r'(?:alldifferent_except_0|all_different_except_0|alldifferent_except)\s*\(', content):
                return True
        except Exception:
            pass
    # FZN ファイルでも検索（globals 経由で間接的に使われる場合）
    for f in prob_dir.glob("*.fzn"):
        try:
            content = f.read_text()
            if re.search(r'(?:sabori_alldifferent_except_0|alldifferent_except_0|all_different_except_0)', content):
                return True
        except Exception:
            pass
    return False
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_alldiff_except_0_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: alldifferent_except_0 Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: alldifferent_except_0 Problems</h1>
    <div class="meta">
...
def main():
    ...
    # alldifferent_except_0 を含む問題を全年度から収集
    problems = []
    prob_years = {}
    ...
    for year, probs_dir in sorted(PROB_DIRS.items()):
        ...
        for p in sorted(probs_dir.iterdir(), key=lambda x: natural_sort_key(x.name)):
            if not p.is_dir():
                continue
            if has_alldifferent_except_0(p):
                prob_name = p.name
    ...
    print(f"Found {len(problems)} problems with alldifferent_except_0 constraint:")
    for prob in problems:
