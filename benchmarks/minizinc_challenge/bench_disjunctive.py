#!/usr/bin/env python3
"""Sabori vs CP-SAT ベンチマーク比較: disjunctive 制約を含む問題のみ
...
def has_disjunctive(prob_dir):
    """問題ディレクトリが disjunctive を使用しているか"""
        ...
        try:
            ...
            for line in content.splitlines():
                ...
                if stripped.startswith('%'):
                    continue
                if re.search(r'\bdisjunctive(?:_strict)?\s*\(', stripped):
                    return True
                if re.search(r'include\s+"disjunctive(?:_strict)?\.mzn"', stripped):
                    return True
        except Exception:
            pass
    return False
...
def generate_html(results, problems, output_path=None, prob_types=None, data_labels=None, prob_years=None):
    ...
    if output_path is None:
        ...
        output_path = f"sabori_benchmark_disjunctive_{ts}.html"
...
<head>
    <meta charset="UTF-8">
    <title>Sabori vs CP-SAT: disjunctive Problems</title>
    <style>
        body {{
...
<body>
    <h1>Sabori vs CP-SAT: disjunctive Problems</h1>
    <div class="meta">
...
def main():
    ...
    print(f"Found {len(problems)} problems with disjunctive constraint:")
    for prob in problems:
