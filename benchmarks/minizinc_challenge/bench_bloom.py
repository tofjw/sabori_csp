#!/usr/bin/env python3
"""NoGood-Bloom 重なりタイブレーク A/B test.

Same binary, toggled via env SABORI_BLOOM:
  - bloom_on  : SABORI_BLOOM=1  -> MRV/activity 同点時に NoGood-Bloom 重なりでタイブレーク（既定動作）
  - bloom_off : SABORI_BLOOM=0  -> タイブレーク無効（ng_usage_bloom を蓄積しない）

測りたい問い: Bloom タイブレーク（記事2章）は探索性能に寄与しているか。
...
PAIRS = [
    ("bloom_on", "bloom_off"),  # Bloom タイブレークの寄与
]
