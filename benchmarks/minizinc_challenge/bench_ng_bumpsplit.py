#!/usr/bin/env python3
"""NoGood/決定変数の activity bump をさらに細かく分離する ablation。

Same binary, env で切替（既定=全部 on）:
  - full        : 既定
  - learn_off   : SABORI_NG_LEARN_BUMP=0 -> NoGood 学習時 bump（:223, 0.01 スケール）のみ off
  - prop_off    : SABORI_NG_PROP_BUMP=0  -> NoGood 伝播時 bump（:74/100/122, フルスケール/n）のみ off
  - decvar_off  : SABORI_DECVAR_BUMP=0   -> 決定変数 bump（handle_failure, フルスケール常時ON）off

測りたい問い: §3 の「NoGood は activity 経由で効く」+18 の内訳は、学習時 bump（極小0.01）か伝播時 bump か。
さらに、最大かつ未計測だった決定変数 bump はどれだけ効いているか。
各 full vs X で X の寄与を測る（full が勝てば X の bump は効いている）。
"""
...
# pairwise: full vs each (full が勝てば その bump が効いている)
PAIRS = [
    ("full", "learn_off"),   # 学習時 bump(0.01) の寄与
    ("full", "prop_off"),    # 伝播時 bump(フルスケール) の寄与
    ("full", "decvar_off"),  # 決定変数 bump の寄与
]
