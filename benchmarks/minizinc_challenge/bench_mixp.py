#!/usr/bin/env python3
"""mix_p A/B test: adaptive bandit (ModeRewardPolicy) vs fixed heuristics.

Same binary, toggled via env SABORI_FIX_MIXP:
  - adaptive : env unset            -> リスタートごとに mix_p をバンディット適応
  - fix_mrv  : SABORI_FIX_MIXP=0    -> 常に MRV 優先に固定
  - fix_act  : SABORI_FIX_MIXP=4    -> 常に Activity 優先に固定
...
def run_fzn_solver(prob, cfg_name, fix_val, fzn_path):
    ...
    env = dict(os.environ)
    if fix_val is None:
        env.pop("SABORI_FIX_MIXP", None)
    else:
        env["SABORI_FIX_MIXP"] = fix_val
    start = time.time()
    is_to = False
    proc = None
    try:
        ...
        if obj is None:
            for line in reversed(err.split('\n')):
                ...
                if m:
                    obj = int(m.group(1)); break
        return (prob, cfg_name, status, max(elapsed, 0.0), obj)
    except Exception:
        return (prob, cfg_name, "ERROR", 0, None)
    finally:
        if proc is not None:
            kill_process_tree(proc)

def cmp_pair(a, b, ptype):
    """adaptive(a) vs fixed(b). returns 'A' / 'B' / 'T'.
...
def main():
    ...
    for prob in problems:
        for cfg_name, fix_val in CONFIGS:
            tasks.append((prob, cfg_name, fix_val, fzn_map[prob]))
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for fut in as_completed(futures):
            prob, cfg, status, elapsed, obj = fut.result()
            results[(prob, cfg)] = (status, elapsed, obj)
            ostr = f" obj={obj}" if obj is not None else ""
            print(f"{prob:32s} {cfg:9s} -> {status:8s} {elapsed:6.2f}s{ostr}")

    # ---- pairwise tally: adaptive vs each fixed ----
    print("\n" + "=" * 90)
    print(f"{'Problem':32s} {'type':4s} {'adaptive':>20s} {'fix_mrv':>20s} {'fix_act':>20s}")
    print("-" * 110)
    tally = {"fix_mrv": [0, 0, 0], "fix_act": [0, 0, 0]}  # [adaptive_win, fixed_win, tie]
    vbest = [0, 0, 0]  # adaptive vs best-of-fixed: [adaptive_win, fixed_win, tie]
    for prob in problems:
        ptype = prob_types.get(prob, "SAT")
        a = results.get((prob, "adaptive"), ("?", 0, None))
        m = results.get((prob, "fix_mrv"), ("?", 0, None))
        k = results.get((prob, "fix_act"), ("?", 0, None))
        def fmt(r):
            return f"{r[0]}" + (f"(obj={r[2]})" if r[2] is not None else "")
        print(f"{prob:32s} {ptype:4s} {fmt(a):>20s} {fmt(m):>20s} {fmt(k):>20s}")
        for fx, r in [("fix_mrv", m), ("fix_act", k)]:
            res = cmp_pair(a, r, ptype)
            tally[fx][0 if res == "A" else (1 if res == "B" else 2)] += 1
        # vs best fixed: adaptive must beat BOTH-fixed-considered-jointly
        rm = cmp_pair(a, m, ptype)
        rk = cmp_pair(a, k, ptype)
        if rm == "B" or rk == "B":
            vbest[1] += 1   # ある固定が adaptive に勝った
        elif rm == "A" or rk == "A":
            vbest[0] += 1   # adaptive がどれかに勝ち、負けなし
        else:
            vbest[2] += 1

    print("\n" + "=" * 90)
    print("PAIRWISE (adaptive 視点):")
    for fx in ("fix_mrv", "fix_act"):
        w, l, t = tally[fx]
        print(f"  adaptive vs {fx:8s}:  adaptive_win={w}  {fx}_win={l}  tie={t}")
    print(f"  adaptive vs best-of-fixed:  adaptive_win={vbest[0]}  fixed_win={vbest[1]}  tie={vbest[2]}")
    print(f"\nProblems: {len(problems)}  Timeout: {TIMEOUT}s  Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
