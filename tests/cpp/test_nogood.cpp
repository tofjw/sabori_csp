...
TEST_CASE("Learning L0: bounds_at reconstructs bounds at inference time", "[learning]") {
    ...
    // (record_inference 経由で var_trail_index_ も構築する)
    solver.set_learning(true);
    solver.record_inference(static_cast<uint32_t>(x->id()), 3,
                            Literal::Type::Geq, 0);
    solver.record_inference(static_cast<uint32_t>(x->id()), 8,
                            Literal::Type::Leq, 0);
    solver.record_inference(static_cast<uint32_t>(x->id()), 5,
                            Literal::Type::Geq, 0);
