...
| `bool_lt_reif` | `IntLinLeReifConstraint` | (a < b) <-> r |
...
| `bool_xor` | `BoolXorConstraint` | c = (a XOR b) |
...
| `sabori_subcircuit` | `SubcircuitConstraint` | 非自己ループのノードがちょうど1つの閉路を形成（x[i]=i は不参加）。alldifferent は registry が併設。fzn_subcircuit を経路付け |
